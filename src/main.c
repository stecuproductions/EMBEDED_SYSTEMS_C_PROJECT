...
void playMySong(void) {
    const char* melody = "C4,E4,G4,C5,";   //C 400 S E 400 S G 400 S ITD...

    while (*melody != '\0') {
        uint32_t note = getNote(*melody++); //Czytamy znak (C i posinkrementujemy)
        uint32_t duration = 400;  //duration (losowa liczba, zaraz ja ustawimy)

        if (*melody >= '0' && *melody <= '9') {
            duration = (*melody++ - '0') * 100;  // n '4' = 400 ms
        }
